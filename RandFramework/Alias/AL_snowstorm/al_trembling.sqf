...
if (player_act_cold) then {
    ...
    ["FilmGrain", 2000] spawn {
        params [["_name", ""], ["_priority", 0], ["_effect", []], ["_handle", -1]];
        while {_handle = ppEffectCreate [_name, _priority];
        _handle < 0} do {
    ...
    sleep 1;
    _tremurici = selectRandom ["tremurat_1", "tremurat_2", "NoSound", "tremurat_3", "tremurat_4", "rafala_8"];
    playSound _tremurici;addCamShake [0.5, (snow_gust#1)*2, 25]
};
sleep ((snow_gust select 1) / 2);
