...
SupplySupportChopperOptions     = { _veh = ["B_Heli_Transport_03_unarmed_F"]; if (count TRGM_VAR_WestHelos > 0) then { _veh = TRGM_VAR_WestHelos; }; if (count (TRGM_VAR_WestHelos select {_x call TRGM_GLOBAL_fnc_isTransport}) > 0) then { _veh = (TRGM_VAR_WestHelos select {_x call TRGM_GLOBAL_fnc_isTransport}); }; _veh; };
...
FriendlyMedicalTruck            = { _veh = ["B_Truck_01_medical_F"]; if (count TRGM_VAR_WestUnarmedCars > 0) then { _veh = TRGM_VAR_WestUnarmedCars; }; if (count (TRGM_VAR_WestUnarmedCars select {_x call TRGM_GLOBAL_fnc_isMedical}) > 0) then { _veh = (TRGM_VAR_WestUnarmedCars select {_x call TRGM_GLOBAL_fnc_isMedical}); }; _veh; };
...
FriendlyFuelTruck               = { _veh = ["B_Truck_01_fuel_F"]; if (count TRGM_VAR_WestUnarmedCars > 0) then { _veh = TRGM_VAR_WestUnarmedCars; }; if (count (TRGM_VAR_WestUnarmedCars select {_x call TRGM_GLOBAL_fnc_isFuel}) > 0) then { _veh = (TRGM_VAR_WestUnarmedCars select {_x call TRGM_GLOBAL_fnc_isFuel}); }; _veh; };

FriendlyJet                     = { _veh = ["B_Plane_Fighter_01_Stealth_F"]; if (count TRGM_VAR_WestPlanes > 0) then { _veh = TRGM_VAR_WestPlanes; }; if (count (TRGM_VAR_WestPlanes select {_x call TRGM_GLOBAL_fnc_isArmed && !(_x call TRGM_GLOBAL_fnc_isTransport)}) > 0) then { _veh = (TRGM_VAR_WestPlanes select {_x call TRGM_GLOBAL_fnc_isArmed && !(_x call TRGM_GLOBAL_fnc_isTransport)}); }; _veh; };
FriendlyChopper                 = { _veh = ["B_Heli_Transport_01_camo_F","B_Heli_Attack_01_F"]; if (count TRGM_VAR_WestHelos > 0) then { _veh = TRGM_VAR_WestHelos; }; if (count (TRGM_VAR_WestHelos select {_x call TRGM_GLOBAL_fnc_isArmed && !(_x call TRGM_GLOBAL_fnc_isTransport)}) > 0) then { _veh = (TRGM_VAR_WestHelos select {_x call TRGM_GLOBAL_fnc_isArmed && !(_x call TRGM_GLOBAL_fnc_isTransport)}); }; _veh; };
