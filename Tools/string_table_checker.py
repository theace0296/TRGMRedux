...
def get_all_languages(projectpath):
    ...
    for module in os.listdir(projectpath):
        ...
        stringtablepath = os.path.join(projectpath, module) if module == "stringtable.xml" else os.path.join(
            projectpath, module, "stringtable.xml")
        if not os.path.exists(stringtablepath):
            continue
        try:
...
def main():
    ...
    for module in os.listdir(projectpath):
        ...
        for i in range(len(localized)):
            if "--markdown" not in sys.argv:
                print("  %s %s / %i" %
                      ((languages[i]+":").ljust(10), str(localized[i]).ljust(3), keynumber))
        ...
        for i in range(len(languages)):
            if localizedsum[i] == keysum:
                ...
                print("%s %s missing stringtable entry/entries." %
                      ((languages[i] + ":").ljust(12), str(keysum - localizedsum[i]).rjust(4)), end="")
